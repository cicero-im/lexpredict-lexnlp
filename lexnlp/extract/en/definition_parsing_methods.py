...
PAREN_QUOTE_PTN = (
    rf"""\((?:each(?:,)?\s+)?(?:(?:{join_collection(ARTICLES)})\s+)?['"“](.{{1,{MAX_TERM_CHARS}}}?)\.?['"”]\)"""
)
...
PAREN_PTN = (
    rf"""\((?:E|each(?:,)?\s+)?(?:(?:{join_collection(ARTICLES)})\s+)?([A-Z][^\)]{{1,{MAX_TERM_CHARS}}}?)\.?\)"""
)
...
\s+(?=(?:{trigger_list})\W)
""".format(noun_ptn_base=NOUN_PTN_BASE, trigger_list="|".join([w.replace(" ", r"\s+") for w in STRONG_TRIGGER_LIST]))
...
ANCHOR_SUBJECT_QUOTES_PTN = (
    rf"(?:(?:{join_collection(ANCHOR)})\s+?)(?:.{{1,{MAX_TERM_CHARS}}}\s+?)(?:(?:{join_collection(ARTICLES)})\s+)?"
    rf"(('(.{{1,{MAX_TERM_CHARS}}}?)')|"
    rf"(\"(.{{1,{MAX_TERM_CHARS}}}?)\")|"
    rf"(“(.{{1,{MAX_TERM_CHARS}}}?)”))"
)
...
ABBREVIATION_PTRN = "|".join([a.replace(".", "\\.") for a in EnLanguageTokens.abbreviations])
...
def get_definition_list_in_sentence(
    sentence_coords: tuple[int, int, str], decode_unicode=True
    ...
    sentence = TextBeautifier.unify_quotes_braces(sentence, empty_replacement=" ")
    ...
    for term, start, end in result:
        ...
        split_definitions_lst = split_definitions_inside_term(term, sentence_coords, start, end)
...
def split_definitions_inside_term(
    term: str, src_with_coords: tuple[int, int, str], term_start: int, term_end: int
...
def trim_defined_term(term: str, start: int, end: int) -> tuple[str, int, int, bool]:
    """
    ...
    term, start, end = TextBeautifier.strip_string_coords(term, start, end, STRIP_PUNCT_SYMBOLS)
...
def regex_matches_to_word_coords(pattern: re.Pattern, text: str, phrase_start: int = 0) -> list[tuple[str, int, int]]:
    ...
    """
    return [(m.group(), m.start() + phrase_start, m.end() + phrase_start) for m in pattern.finditer(text)]
