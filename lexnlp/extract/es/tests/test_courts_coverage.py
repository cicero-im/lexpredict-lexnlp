"""Coverage tests for the deprecated es _get_courts helper and list wrapper."""
...
from lexnlp.extract.es.courts import _get_courts, get_court_annotation_list, get_court_annotations


def _entries() -> list[DictionaryEntry]:
    return [DictionaryEntry(id=1, name="Tribunal Superior de Justicia de Madrid")]


def test_get_courts_warns_and_yields_match() -> None:
    text = "El actual Tribunal Superior de Justicia de Madrid fue creado en 1985."
    ...
    assert entry.name == "Tribunal Superior de Justicia de Madrid"
    assert entry.id == 1
    assert alias.alias == "Tribunal Superior de Justicia de Madrid"


def test_get_courts_no_match_yields_nothing() -> None:
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", DeprecationWarning)
        assert list(_get_courts("hola mundo sin juzgados", _entries())) == []
...
def test_get_court_annotation_list() -> None:
    ...
    assert result[0].name == "Tribunal Superior de Justicia de Madrid"
    assert result[0].locale == "es"
    assert result[0].coords == (9, 49)
    ...
    assert get_court_annotation_list("hola mundo sin juzgados") == []
