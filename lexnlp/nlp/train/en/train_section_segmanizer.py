...
SECTION_BREAK_POSITIONS = {
    ...
    "/samples/agreements/construction/1011109_2000-08-14_CONSTRUCTION AGREEMENT (BALLANTYNE).txt": [
        ...
        335,
        426,
    ],  # the last section is also 'Exhibit...'
    "/samples/agreements/employment/1046578_2006-08-08_EX-10.32(A) AMENDMENT TO EMPLOYMENT AGREEMENT.txt": [
        58,
        77,
        187,
    ],
...
class SectionSegmentizerTrainManager:
    ...
    def build_features(self, samples_repository_path: str, target_path: str):
        section_break_positions = ensure_documents_in_folder(
            SECTION_BREAK_POSITIONS, target_path, OrderedDict({"/samples/": samples_repository_path})
        )
        ...
        for file_name in sorted(section_break_positions):
            ...
            for line_id in range(len(file_lines)):
                self.feature_data.append(
                    self._build_section_break_features(file_lines, line_id, include_doc=doc_distribution)
                )
    ...
    @classmethod
    def _build_document_distribution(cls, text: str, characters=string.printable, norm=True):
        """
    ...
    def _build_section_break_features(self, lines, line_id, characters=string.printable, include_doc=None):
        """
