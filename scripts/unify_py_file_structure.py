...
author_ptn = re.escape(author.strip()).replace(r"0\.0\.0", r"\d\.\d\.\d+")
...
def unify_file_structure(release_number):
    ...
    files = sorted([os.path.join(a, i) for path in paths for a, _, b in os.walk(path) for i in b if i.endswith(".py")])
    if exclude_paths:
        files = files + [
            os.path.join(base_dir, i) for i in os.listdir(base_dir) if i.endswith(".py") and i not in exclude_paths
        ]
